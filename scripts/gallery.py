"""Regenerate the docs component gallery: screenshots plus the page itself.

Starts a real Leika server, drives the built browser client with Playwright,
and captures every GUI component in light and dark. Each entry's snippet is
the exact code that was executed for its screenshot -- the registry below is
the single source of truth, and ``docs/gallery.md`` is written from it so the
page can never drift from the images.

Run with ``make gallery`` (or ``python scripts/gallery.py``) whenever the
client's look changes; the PNGs are checked in because the CI docs build has
no browser.
"""
...
# The union of the element's own rect and every visible descendant's. A
# layout box alone under-reports components whose ink overflows it -- a
# slider's thumb rides above its row -- and a clip taken from it beheads
# them. Two guards keep the union honest: fixed-position descendants are no
# part of a row's ink (Base UI parks a 1x1 hidden native input at the page
# origin, which would stretch the union across the whole panel), and the
# overflow is capped at the pad distance, because some ink is invisible --
# plotly lays 20px hover zones past its corners -- and an uncapped union
# would grow the frame toward whatever chrome sits next to the row.
_INK_BOX_JS = """
(el, pad) => {
...
def write_page(out_dir: Path) -> None:
    """Write docs/gallery.md: a grid of tiles per group, in raw HTML.

    Raw rather than MyST constructs because the page is an application of the
    docs' own stylesheet -- each tile is one anchor to the component's API
    entry with its theme-synced screenshot pair, styled by `gallery-card` in
    shadcn.css.
    """
    doc = out_dir.parent.parent / "gallery.md"
    lines = [
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "docs" / "_static" / "gallery",
    ...
    try:
        with sync_playwright() as p:
            ...
            # The settings gear and connection badge sit just above the
            # generated GUI; a capture padded upward would catch their
            # bottom edge as a cropped sliver. They are panel chrome, not
            # component, so they sit out the shoot.
            page.add_style_tag(
            ...
            for _, entries in ENTRIES:
                for entry in entries:
                    ...
                    # Locate once and shoot both themes before dismissing:
                    # scheme switches repaint in place, and a dismissed toast
                    # would not come back for the second shot.
    ...
    print(f"captured {total} components into {args.out.relative_to(ROOT)}")
    return 0
